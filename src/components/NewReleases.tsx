...
import { getNewReleases } from '../services/mangaService';
...
export const NewReleases = () => {
  ...
  useEffect(() => {
    ...
    const fetchNewReleases = async () => {
      try {
        setLoading(true);
        const mangasWP = await getNewReleases();
        if (mangasWP.length > 0) {
          setItems(mangasWP.map(manga => ({
            id: manga.id,
            title: manga.titulo,
            coverImage: manga.portada,
            rating: 10,
    ...
    <section className="relative w-full py-10 z-20">
      <div className="w-full max-w-[1600px] mx-auto px-6 lg:px-12">
        ...
        <div className="flex items-center gap-3 mb-8 border-l-4 border-violet-600 pl-4">
            <h2 className="text-3xl md:text-4xl font-[900] text-white uppercase italic tracking-tighter flex items-center gap-2">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 h-auto lg:h-[600px]">
            ...
            <Link 
                ...
                className="group relative col-span-1 lg:col-span-2 h-[500px] lg:h-full block overflow-hidden bg-[#111]"
            ...
            >
                ...
                <div className="absolute bottom-0 left-0 w-full p-8 md:p-12 flex flex-col items-start gap-4">
                    ...
                    <h3 className="text-3xl md:text-5xl lg:text-6xl font-[900] text-white uppercase italic leading-none line-clamp-2 drop-shadow-xl" style={{ textShadow: '4px 4px 0 #000' }}>
                        {mainItem.title}
                    </h3>
                    ...
                    <div className="max-w-xl hidden md:block">
                        <p className="text-gray-300 font-medium text-base leading-snug line-clamp-3 drop-shadow-md">
                            {mainItem.synopsis}
                        </p>
                    </div>
                </div>
            ...
            <div className="col-span-1 grid grid-cols-2 gap-4 h-full content-start">
                {sideItems.map((item, index) => (
                    <Link 
                        key={item.id} 
                        to={`/manga/${item.id}`}
                        className="group relative block overflow-hidden bg-[#111] h-[240px] lg:h-[290px]"
                    ...
                    >
                        ...
                        <div className="absolute bottom-0 left-0 w-full p-3 md:p-4">
                            <h4 className="text-white text-sm md:text-lg font-black uppercase italic leading-none mb-1 group-hover:text-violet-400 transition-colors line-clamp-2 drop-shadow-md">
                                {item.title}
                            </h4>
