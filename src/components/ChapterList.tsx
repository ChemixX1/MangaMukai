...
import { buyChapter } from "../utils/auth";
...
export const ChapterList = ({ chapters, purchasedChapterIds, userCoins, userInfo, onPurchaseSuccess }: ChapterListProps) => {
  ...
  const paidChapters = chapters.filter(c => c.is_paid).length;
  const freeChapters = totalChapters - paidChapters;
  ...
  const isChapterLocked = (chapter: Chapter) => {
    ...
    if (purchasedChapterIds.has(idStr) || localUnlocked.has(idStr)) {
      return false;
    }
    return true;
  };
      ...
      <div className="w-[94%] mx-auto mb-32 relative z-20">
        ...
        <div className="bg-[#0A0A0A] border-y border-white/5">
          <div className="hidden md:grid grid-cols-12 bg-[#111] py-3 px-4 text-[9px] uppercase font-bold text-white/30 tracking-[0.2em]">
            ...
            <div className="col-span-4 text-center">Disponible Gratis</div>
          ...
          <div className="flex flex-col">
            {visibleChapters.map((chapter) => {
              const locked = isChapterLocked(chapter);
              const isPurchased = purchasedChapterIds.has(String(chapter.id)) || localUnlocked.has(String(chapter.id));
                  ...
                  <div className="hidden md:flex col-span-4 justify-center items-center">
                    {locked && isFutureFree ? (
                      ...
                      <span className="text-[10px] text-white/10 uppercase font-bold tracking-widest">
                        {chapter.is_paid && !isFutureFree ? "Premium" : "-"}
                      </span>
                    )}
                  </div>
