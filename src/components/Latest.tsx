...
import { Grid, Clock, Flame, Star } from "lucide-react";
import { Link } from "react-router-dom";
import { getLatestWomenUpdates } from '../services/mangaService';
...
export const Latest = () => {
  ...
  useEffect(() => {
    ...
    if (latestWomen.length > 0) {
      setItems(mapToItems(latestWomen));
      setLoading(false);
    }

    // Siempre refrescar desde el endpoint (tiene cache 5 min en servidor)
    const fetchMangas = async () => {
      try {
        const data = await getLatestWomenUpdates(18);
        if (data.length > 0) setItems(mapToItems(data));
      } catch (error) {
        console.error(error);
      } finally {
        setLoading(false);
    ...
    const timer = setInterval(fetchMangas, 5 * 60 * 1000);
    ...
    <section className="relative w-full pb-10 z-20">
      ...
      <div className="w-full max-w-[1600px] mx-auto px-4 md:px-6 lg:px-12 pt-6">
        ...
        <div className="flex justify-between items-end mb-8">
             ...
             <Link to="/catalog" className="hidden md:flex items-center gap-2 px-6 py-2 rounded bg-[#1a1a1a] text-white font-bold uppercase text-sm hover:text-[#FF4D88] transition-colors border border-transparent hover:border-[#FF4D88]">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-x-3 gap-y-8 md:gap-x-4 md:gap-y-10">
            {items.map((manga, index) => (
              ...
              >
                  {/* PORTADA */}
                  <div className="relative aspect-[3/4.4] rounded-lg overflow-hidden mb-3 md:mb-4 bg-[#0f1115] shadow-lg group-hover:shadow-2xl group-hover:shadow-[#FF4D88]/20 transition-all duration-300 group-hover:-translate-y-2 shine-effect border border-white/5">
                  ...
                  <div className="flex flex-col flex-grow items-center text-center"> 
                    <h3 className="text-[13px] md:text-[15px] font-[800] text-white leading-tight line-clamp-2 mb-2 md:mb-3 uppercase tracking-tight group-hover:text-[#FF4D88] transition-colors font-sans">
                      {manga.title}
                    </h3>
                    ...
                    <div className="mt-auto w-full space-y-1">
                        {manga.chapters.slice(0, 2).map((cap) => (
                            <Link 
                                key={cap.id} 
                                to={`/chapter/${cap.id}`} 
                                className="grid grid-cols-[auto_1fr_auto] gap-2 md:gap-3 items-center w-full bg-[#161616] hover:bg-[#222] px-2 md:px-3 py-1.5 md:py-2 rounded-md border border-white/5 hover:border-white/10 transition-all group/cap"
                            >
                                {/* NÚMERO */}
                                <div className="flex items-center min-w-0">
                                    <span className="text-gray-200 text-[10px] md:text-[12px] font-bold group-hover/cap:text-[#FF4D88] transition-colors whitespace-nowrap overflow-hidden text-ellipsis">
                                        Ch. {cap.numero}
                                    </span>
                                </div>
                                ...
                                <div className="flex items-center justify-end shrink-0">
                                    <span className={`text-[9px] md:text-[10px] font-bold whitespace-nowrap ${cap.dateLabel === 'New' ? 'text-[#FF4D88] animate-pulse flex items-center gap-0.5' : 'text-gray-400'}`}>
