...
import coinLogo from "../assets/logosnovedades/logocoin.png";
...
export const CoinMarketModal = ({ isOpen, onClose, username }: Omit<CoinMarketModalProps, 'userId'> & { userId?: string }) => {
  ...
  const cardClass = "relative flex flex-col justify-between p-4 bg-[#02040a] border border-white/10 rounded-xl transition-all duration-300 group hover:border-[#FF4D88] hover:shadow-[0_0_15px_rgba(255,77,136,0.15)] overflow-hidden text-left h-[136px]";
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      const res = await fetch('https://mangamukai.com/wp-json/mangamukai/v1/buy-coins', {
        method: 'POST',
        headers: {
  ...
  return (
    <div className="fixed inset-0 z-[300] flex items-center justify-center p-4 bg-black/95 backdrop-blur-sm">
      <motion.div
        ...
        className="relative bg-[#0F1115] w-full max-w-[720px] h-[95vh] max-h-[920px] flex flex-col md:flex-row rounded-none md:rounded-3xl overflow-hidden shadow-2xl border border-white/10"
      >
        {/* LADO IZQUIERDO (DECORATIVO) */}
        <div className="hidden md:flex w-[32%] bg-[#050505] relative flex-col justify-between border-r border-white/5 overflow-hidden">
  ...
  <img src={coinLogo} alt="Coin Decor" className="h-full w-auto object-contain object-bottom drop-shadow-[0_0_40px_rgba(255,77,136,0.15)]" />
        ...
        <div className="w-full md:w-[68%] bg-[#ffffff] relative flex flex-col h-full">
          ...
          <div className="p-8 pt-2 overflow-y-auto custom-scrollbar flex-1 flex flex-col justify-center relative">
            <AnimatePresence mode="wait">
              
              {/* VISTA 1: GRID DE PACKS */}
              {!selectedPack && (
                <motion.div key="grid" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="grid grid-cols-1 sm:grid-cols-2 gap-4 w-full max-w-[500px] mx-auto">
                  {COIN_PACKS.map((pack) => (
              ...
              {selectedPack && (
                <motion.div key="checkout" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} className="w-full max-w-[400px] mx-auto flex flex-col gap-6">
                  
                  <div className="bg-[#02040a] border border-white/10 rounded-xl p-6 flex items-center justify-between shadow-xl">
                    <div>
                  ...
                  <div className="relative z-50 min-h-[100px] flex flex-col justify-center">
                    <button
